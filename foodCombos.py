...
def foundCombos( parse_userRecipes, totalTimeFound, matrix,baseListIngredients):
	...
	twoIngredient = choice(baseListIngredients, 2, p=weightsOfIngredients)
	...
	label = Label(root,bg='lightblue',text='We Made a Recipe of Size 2').place(x=10,y=190)
	label = Label(root,bg='lightblue',text=twoIngredient).place(x=250,y=190)
	...
	label = Label(root,bg='lightblue',text='Recipes in Our DataBase').place(x=10,y=300)
	label = Label(root,bg='lightblue',text=baseListIngredients).place(x=10,y=320)
...
## pairs in the matrix
	...
	#printi(ng the found Pairs in the user Recipe
	#print (foundPairs
